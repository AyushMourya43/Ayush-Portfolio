      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-20">
          <h2 className="group inline-block text-4xl sm:text-5xl md:text-7xl lg:text-8xl font-black text-gray-900 mb-6 leading-tight whitespace-nowrap transition-all duration-200 hover:-translate-y-0.5 hover:shadow-sm">
            <span className="text-gray-900 transition-colors duration-200 group-hover:text-orange-600">Featured</span>{' '}
            <span className="text-orange-600 transition-colors duration-200 group-hover:text-gray-900">Projects</span>
          </h2>
          <div className="max-w-4xl mx-auto">
            ...
            <div className="flex justify-center items-center space-x-8 mb-8">
              ...
              <div className="flex space-x-4">
                <ExternalLink className="text-orange-500 animate-pulse" size={32} />
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {projects.map((project, index) => (
            <div key={index} className="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 group">
                ...
                <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-4">
                  {project.link !== '#' && (
                    <a 
                      ...
                      className="p-2 bg-white rounded-full text-gray-900 hover:text-orange-600 transition-colors"
                    >
              ...
              <div className="p-6">
                <h3 className="text-xl font-bold text-gray-900 mb-2 group-hover:text-orange-600 transition-colors">{project.title}</h3>
                <p className="text-gray-600 mb-4 text-sm line-clamp-3">{project.description}</p>
                <div className="flex flex-wrap gap-2">
                  {project.tags.map((tag) => (
                    <span key={tag} className="px-3 py-1 bg-orange-50 text-orange-600 rounded-full text-xs font-medium">
                      {tag}
                    </span>
                  ))}
                </div>
