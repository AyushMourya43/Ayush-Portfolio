...
const Experience = () => {
  ...
  const renderCard = (exp) => (
    <div className="bg-gray-50 p-6 rounded-xl shadow-sm hover:shadow-lg hover:-translate-y-1 hover:scale-105 transition-all duration-300 border border-gray-100 cursor-pointer h-full">
      <div className="flex items-center justify-between mb-4">
        ...
        {exp.type && <span className="text-xs font-semibold px-2 py-1 bg-orange-100 text-orange-700 rounded-full">{exp.type}</span>}
      </div>
      <h4 className="text-orange-600 font-medium mb-2 flex items-center gap-2">
  ...
  const renderUpcoming = () => (
    <div className="bg-gradient-to-br from-orange-50 to-white p-6 rounded-xl shadow-sm hover:shadow-lg hover:-translate-y-1 hover:scale-105 transition-all duration-300 border border-orange-100 cursor-pointer h-full">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-xl font-bold text-orange-800">Upcoming Opportunity</h3>
        <span className="text-xs font-semibold px-2 py-1 bg-orange-100 text-orange-700 rounded-full">Upcoming</span>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        <div className="text-center mb-20">
          <h2 className="group inline-block text-4xl sm:text-5xl md:text-7xl lg:text-8xl font-black text-gray-900 mb-6 leading-tight whitespace-nowrap transition-all duration-200 hover:-translate-y-0.5 hover:shadow-sm">
            <span className="text-gray-900 transition-colors duration-200 group-hover:text-orange-600">Work</span>{' '}
            <span className="text-orange-600 transition-colors duration-200 group-hover:text-gray-900">Experience</span>
          </h2>
          <div className="max-w-4xl mx-auto">
            ...
            <div className="flex justify-center items-center space-x-8 mb-8">
              ...
              <div className="flex space-x-4">
                <Briefcase className="text-orange-500 animate-pulse" size={32} />
          ...
          {Array.from({ length: Math.ceil(experiences.length / 2) }).map((_, rowIdx) => {
            ...
            return (
              <div key={rowIdx} className="relative">
                ...
                <div className="hidden md:block absolute top-0 left-1/2 -translate-x-1/2 bg-orange-600 w-4 h-4 rounded-full border-4 border-white shadow-sm z-10"></div>
